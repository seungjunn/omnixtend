...
import chipyard.{HasHarnessSignalReferences, BuildTop, ChipTop, ExtTLMem, CanHaveMasterTLMemPort, CanHaveMasterTLOXPort, DefaultClockFrequencyKey}
...
// DOC include end: UartOverlay
  ...
  // Loading AXI4-Lite and TileLink nodes from ethoverlayOutput
  ...
  // Locate the AXI4-Lite node in ChipTop and assign it in ethClient
  ...
  // Connecting the AXI4-Lite Bus between ethoverlayOutput and ChipTop
  ethNode := ethClient

  // Locate the Tilelink node in ChipTop and assign it in oxClient
  val inParams3 = topDesign match { case td: ChipTop =>
    td.lazySystem match { case lsys: CanHaveMasterTLOXPort =>
      lsys.OXTLNode.edges.in(0)
    }
  }
  val oxClient = TLClientNode(Seq(inParams3.master))
  // Connecting the Tilelink Bus between ethoverlayOutput and ChipTop
