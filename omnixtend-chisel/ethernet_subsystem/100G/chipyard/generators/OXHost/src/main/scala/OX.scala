...
class OmniXtendNode(implicit p: Parameters) extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val packetgenerator = Module(new PacketGenerator)

    val thr_cnt = RegInit(0.U(10.W))
    val thr_valid = RegInit(false.B)
    val rx_cnt = RegInit(0.U(10.W))
    val rx_valid = RegInit(false.B)
    val test_mode = RegInit(false.B)
    when(io.debug1){
      test_mode := !test_mode
    }

    packetgenerator.io.test_mode := test_mode
    packetgenerator.io.m_axis_tready := io.txready
    packetgenerator.io.rx_axis_tvalid := io.rxvalid
    when(test_mode){
      io.txdata := packetgenerator.io.m_axis_tdata + thr_cnt + rx_cnt
      io.txvalid := packetgenerator.io.m_axis_tvalid
      io.txlast := packetgenerator.io.m_axis_tlast
      io.txkeep := packetgenerator.io.m_axis_tkeep
    }.otherwise{
    ...
    thr_cnt := packetgenerator.io.o_thr_cnt
    thr_valid := packetgenerator.io.o_thr_valid
    rx_cnt := packetgenerator.io.o_rx_cnt
    rx_valid := packetgenerator.io.o_rx_valid
