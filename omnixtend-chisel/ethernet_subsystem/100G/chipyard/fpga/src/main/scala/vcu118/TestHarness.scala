...
// DOC include end: UartOverlay
  ...
  val regdataNode = ethoverlayOutput.regdata
  ...
  val inParams4 = topDesign match { case td: ChipTop =>
    td.lazySystem match { case lsys: CanHaveMasterTLMMIOPort =>
      lsys.mmioTLNode.edges.in(0)
    }
  }
  val regdataClient = TLClientNode(Seq(inParams4.master))
  val regconnectorNode = TLIdentityNode()
  regconnectorNode := regdataClient
  regdataNode :=* regconnectorNode
