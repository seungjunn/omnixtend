...
class WithREGIOPassthrough extends OverrideIOBinder({
  (system: CanHaveMasterTLMMIOPort) => {
    val io_tl_mem_pins_temp = IO(DataMirror.internal.chiselTypeClone[HeterogeneousBag[TLBundle]](system.mmio_tl)).suggestName("reg_slave")
    io_tl_mem_pins_temp <> system.mmio_tl
    (Seq(io_tl_mem_pins_temp), Nil)
  }
})
